...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='Made By Alideew1234', icon_url='https://i.imgur.com/r0tFkIq.jpg')
        e.set_author(name=self.user.name, url='https://ghostbin.com/paste/v6jj2', icon_url=self.user.avatar_url)
        return e
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run `{}help [command]`\n'
                                 'For further help, see https://ghostbin.com/paste/v6jj2/').format(prefix)
        if not self.is_all:
